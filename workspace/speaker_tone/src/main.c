...
static uint8_t * song1 = (uint8_t*)"C2,C2,G2,G2,A2,A2,G4,F2,F2,E2,E2,D2,D2,C4,G2,G2,F2,F2,E2,E2,D4,G2,G2,F2,F2,E2,E2,D4,C2,C2,G2,G2,A2,A2,G4,F2,F2,E2,E2,D2,D2,C4,";
static uint8_t * song2 = (uint8_t*)"E2,E2,F2,G2,G2,F2,E2,D2,C2,C2,D2,E2,E4,D4,E2,E2,F2,G2,G2,F2,E2,D2,C2,C2,D2,E2,D3,C1,C2,";
static uint8_t * song3 = (uint8_t*)"C2.C2,D4,C4,F4,E8,";
static uint8_t * song4 = (uint8_t*)"C2.C2,D4,C4,F4,E8,";
...
int main (void) {
    ...
    uint8_t joy = 0; //inicializo la variable joy con 0
    joystick_init(); //inicialo el Joystick
    ...
    while (1)
    {
    	...
    	if((joy & JOYSTICK_DOWN) !=0){
            playSong((uint8_t*)song1);
            Timer0_Wait(3000);
    	}
    	if((joy & JOYSTICK_UP) !=0){
            playSong((uint8_t*)song2);
            Timer0_Wait(3000);
    	}
    	if((joy & JOYSTICK_LEFT) !=0){
            playSong((uint8_t*)song3);
            Timer0_Wait(3000);
    	}
        if((joy & JOYSTICK_RIGHT) !=0){
        	playSong((uint8_t*)song4);
        	Timer0_Wait(3000);
        }
    }
